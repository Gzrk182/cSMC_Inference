...
FHN_LT_bridge_2_list_gen<-function(V, par_vec, T_step){
  T_step_0<-T_step
  #initiallize
  ...
  potential_0<-function(x,y){
    ...
    return(mvnfast::dmvn(V[1],eAdelta_0[1,1]*f_t_1(T_step_0,e,g,b,s_1,s_2,0)+
                           eAdelta_0[1,2]*f_t_2(T_step_0,e,g,b,s_1,s_2,0),
                         Cdelta_0[1,1],log = T))
  }
  ...
  potential_xx_last<-function(V){
    ...
    potential<-function(x,y){
      ...
      return(mvnfast::dmvn(V,mu,Cdelta[1,1],log = T))
    }
...
FHN_LT_bridge_list_gen<-function(V, par_vec, T_step,K){
  T_step_0<-T_step
  #initiallize
  par_name<-c("e","g","b","s_1","s_2")
  for (i in 1:5) {
...
y_8<-matrix(1:((8*(999))*2),ncol=2)
tk_list_2<-FHN_LT_bridge_2_list_gen(V_20_e[-1],append(par_vec[1,1:4],
                                                      0,after = 3),T_step)
tk_list_4<-FHN_LT_bridge_list_gen(V_20_e[-1],append(par_vec[1,1:4],
                                                    0,after = 3),T_step,4)
tk_list_8<-FHN_LT_bridge_list_gen(V_20_e[-1],append(par_vec[1,1:4],
                                                    0,after = 3),T_step,8)


psi_spec_ini_4<-cSMC_4_slow(tk_list_2,tk_list_4,y_2,y_4)[[2]]
psi_spec_0<-psi_spec_ini_4
psi_spec_1<-vector(mode = "list", 8*999)
psi_spec_1[[1]]<-psi_spec_0[[1]]
for (t in 2:(999)) {
  psi_spec_1[[(t-2)*8+2]]<-psi_spec_0[[(t-2)*4+2]]
  psi_spec_1[[(t-2)*8+3]]<-list_extrop(psi_spec_0[[(t-2)*4+2]],psi_spec_0[[(t-2)*4+3]])
  psi_spec_1[[(t-2)*8+4]]<-psi_spec_0[[(t-2)*4+3]]
  psi_spec_1[[(t-2)*8+5]]<-list_extrop(psi_spec_0[[(t-2)*4+3]],psi_spec_0[[(t-1)*4]])
  psi_spec_1[[(t-2)*8+6]]<-psi_spec_0[[(t-1)*4]]
  psi_spec_1[[(t-2)*8+7]]<-list(rep(0,2),diag(1e10,2))
  psi_spec_1[[(t-2)*8+8]]<-list(rep(0,2),diag(1e10,2))
  psi_spec_1[[(t-1)*8+1]]<-psi_spec_0[[(t-1)*4+1]]
}
t<-1000
psi_spec_1[[(t-2)*8+2]]<-psi_spec_0[[(t-2)*4+2]]
psi_spec_1[[(t-2)*8+3]]<-list_extrop(psi_spec_0[[(t-2)*4+2]],psi_spec_0[[(t-2)*4+3]])
...
psi_spec_1[[(t-2)*8+7]]<-list(rep(0,2),diag(1e10,2))
psi_spec_1[[(t-2)*8+8]]<-list(rep(0,2),diag(1e10,2))
pf_result_1<-twist_bpf(100,y_8,tk_list_8,0.5,psi_spec_1)
extract_likelihood(pf_result_1[[2]],pf_result_1[[3]])
sum(pf_result_1[[3]])
#keep doing this
pf_result<-pf_result_1
...
while (ggg<3) {
  ...
  pf_result<-twist_bpf(50,y_8,tk_list_8,0.5,psi_spec)
  if(sum(pf_result[[3]])==0){
    ggg<-ggg+1
  }
  ess<-pf_result[[4]]/50
...
y_m_f<-list(extract_likelihood(pf_result[[2]],pf_result[[3]]),psi_spec)
...
while (i<200) {
  ...
  y_p_f<-cSMC_8(psi_spec_ini,FHN_LT_bridge_list_gen(V_20_e[-1],append((theta_old/scale_par+c_vec[i]*Delta_now)*scale_par,
                                                                      0,after = 3),T_step,n_bridge),y_8,2)
  I_vec<-c(I_vec,y_p_f[[3]])
  if(!all(I_vec<20)){
    break
  }
  psi_spec_ini<-y_p_f[[2]]
  y_m_f<-cSMC_8(psi_spec_ini,FHN_LT_bridge_list_gen(V_20_e[-1],append((theta_old/scale_par-c_vec[i]*Delta_now)*scale_par,
                                                                      0,after = 3),T_step,n_bridge),y_8,2)
  I_vec<-c(I_vec,y_m_f[[3]])
  if(!all(I_vec<20)){
    break
  }
  delta_now<-sample(c(1,-1),4,replace = T,prob = rep(0.5,2))
  psi_spec_ini<-y_p_f[[2]]
  y_p_f_d<-cSMC_8(psi_spec_ini,FHN_LT_bridge_list_gen(V_20_e[-1],append((theta_old/scale_par+d_vec[i]*delta_now+c_vec[i]*Delta_now)*scale_par,
                                                                        0,after = 3),T_step,n_bridge),y_8,2)
  I_vec<-c(I_vec,y_p_f_d[[3]])
  if(!all(I_vec<20)){
    break
  }
  psi_spec_ini<-y_p_f_d[[2]]
  y_m_f_d<-cSMC_8(psi_spec_ini,FHN_LT_bridge_list_gen(V_20_e[-1],append((theta_old/scale_par+d_vec[i]*delta_now-c_vec[i]*Delta_now)*scale_par,
                                                                        0,after = 3),T_step,n_bridge),y_8,2)
  I_vec<-c(I_vec,y_m_f_d[[3]])
  if(!all(I_vec<20)){
    break
  }
  if(!all(I_vec<20)){
  ...
  l_new<-cSMC_8(psi_spec_ini,FHN_LT_bridge_list_gen(V_20_e[-1],append(par_new,
                                                                      0,after = 3),T_step,n_bridge),y_8,2)[[1]]
